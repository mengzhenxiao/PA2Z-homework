...
let original;
let predictionData;
//let say = new P5.Speech(); // speech synthesis object
...
function setup() {
  ...
  select('#enter').mousePressed(saved);
  select('#clean').mousePressed(cleanIt);
}
...
function generate() {
  ...
  if (txt.length > 0) {
    ...
    function gotData(err, result) {
      ...
      select('#prediction').html(predictionData);
    }
  } else {
    // Clear everything
...
function saved() {
    ...
    select('#status').html('Saved!');
    let saved = createP(original+predictionData);
   select('#save').child(saved);
   textInput.value('');
   select('#original').html('');
   select('#prediction').html('');
}
